...
public class UserTest {
	...
	//The password tests should have the same results as in reset password
	//this test has the same issue as reset password - there is no upper limit of 8 characters
	//test 18 too long password - Meaghan Bryant
	@Test
	public void userRegistrationTooLongPasswordTest() {
		try {
		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(5));
		driver.get("http://localhost:/cookingwithlove/");
		driver.findElement(By.linkText("Log in")).click();
	...
	public void userLoginInvalidPasswordTest() {
		...
		//locate alert message on screen
		WebElement errorMessageElement = driver.findElement(By.xpath("//p[contains(.,'Password does not match')]"));
		String errorMessage = errorMessageElement.getText();
